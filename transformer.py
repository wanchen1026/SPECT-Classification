...
from transform import Padding, Interpolate
...
parser.add_argument("--pooling", choices=["last_timestep", "sum"], default="last_timestep")
...
def propagate(loader, model, split, hospital):
    ...
    label_list = []
    with torch.no_grad():
        for image, size, _, _, label in loader:
            image, size = to_device([image, size], device)
            output = model(image, size)
    ...
    return {
        ...
        '%s_loss_%s_DL' % (split, hospital): loss}, pred, label
...
def valid(loaders, model, hospital):
    ...
    for split, loader in loaders.items():
        results, pred, label = propagate(loader, model, split, hospital)
...
def train():
    ...
    for fold in range(args.folds):
        ...
        backend.avgpool = nn.AdaptiveAvgPool2d(args.image_shape)
        ...
        model = MIPModel(
            ...
            drop_transformer=args.drop_transformer,
            drop_cpe=args.drop_cpe,
            pooling=args.pooling,
            image_shape=args.image_shape).to(device)

        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer=optimizer, T_max=args.total_steps)
        ...
        with tqdm(range(args.total_steps), dynamic_ncols=True) as progress:
            for step in progress:
                # train
                model.train()
                image, size, _, _, label = next(infinite_train_loader)
                image, size, label = to_device([image, size, label], device)
                output = model(image, size)
                optimizer.zero_grad()
...
def test():
    ...
    for fold in tqdm(range(args.folds)):
        ...
        for path in paths:
            ...
            sens = recall_score(ckpt['label'], ckpt['pred'], average=None, zero_division=0)
            prec = precision_score(ckpt['label'], ckpt['pred'], average=None, zero_division=0)
            f1 = f1_score(ckpt['label'], ckpt['pred'], average=None, zero_division=0)
            results['%s_%s_accs' % (hospital[:-3], model_name)].append(ckpt['max_acc'])
            results['%s_%s_sens' % (hospital[:-3], model_name)].append(sens)
            results['%s_%s_prec' % (hospital[:-3], model_name)].append(prec)
    ...
    for name, values in results.items():
        ...
        if score == 'accs':
            ...
            msg = '%.4f (%.4f)' % (values.mean(), values.std())
            avg[model_name].update({'test acc': msg})
        else:
            for i, v in enumerate(list(zip(*values))):
                v = np.array(v)
                msg = '%.4f (%.4f)' % (v.mean(), v.std())
                avg[model_name].update({'%s_%d' % (score, i): msg})
    df = pd.DataFrame(avg, columns=sorted(avg.keys()))
    print(args.hospital, '\n', df.to_markdown(), '\n')
